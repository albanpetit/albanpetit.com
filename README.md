...
1. **build** — checks out the repo, installs dependencies with `npm ci`, runs `npm run lint` and `npm run type-check`, runs a clean `npm run build`, and uploads `public/` as a Pages artifact.
